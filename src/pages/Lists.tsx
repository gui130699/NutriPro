...
export function Lists() {
  ...
  return <section className="lists-page">
    ...
    {tab === 'public' && <section className="list-panel">
      <div className="list-panel-heading"><div><h2>Catálogo público</h2><p>{catalog.data?.version.totalFoods ? `${catalog.data.version.totalFoods.toLocaleString('pt-BR')} alimentos disponíveis no seu dispositivo.` : 'O catálogo será disponibilizado assim que a base CSV for importada.'}</p></div>{catalog.data?.stale && <span className="catalog-stale">Usando versão offline</span>}</div>
      ...
      {!catalog.isLoading && !catalog.isError && !publicFoods.length && <EmptyState title="Base de alimentos ainda não importada" detail="Anexe o CSV oficial e execute npm run catalog:import para criar o catálogo local. Nenhum alimento fictício foi incluído." />}
...
function FoodDetailsDialog({ food, onClose }: { food: Food | null; onClose: () => void }) {
  if (!food) return null
  return <div className="editor-backdrop" role="presentation" onMouseDown={onClose}><section className="food-details-dialog" role="dialog" aria-modal="true" aria-labelledby="food-details-title" onMouseDown={(event) => event.stopPropagation()}><button type="button" className="editor-close" aria-label="Fechar detalhes" onClick={onClose}><X size={17} /></button><p className="eyebrow">{food.isPublic ? 'Lista pública' : 'Alimento particular'}</p><h2 id="food-details-title">{food.name}</h2><p>{food.category || 'Sem categoria'}{food.brand ? ` · ${food.brand}` : ''}</p><dl><div><dt>Calorias</dt><dd>{formatNumber(food.calories)} kcal</dd></div><div><dt>Proteínas</dt><dd>{formatNumber(food.protein)} g</dd></div><div><dt>Carboidratos</dt><dd>{formatNumber(food.carbs)} g</dd></div><div><dt>Gorduras</dt><dd>{formatNumber(food.fat)} g</dd></div><div><dt>Fibras</dt><dd>{formatNumber(food.fiber)} g</dd></div><div><dt>Unidade-base</dt><dd>{food.baseUnit === 'ml' ? '100 ml' : '100 g'}</dd></div></dl>{food.source && <p className="food-detail-source">Fonte: {food.source}</p>}{food.notes && <p className="food-detail-notes">{food.notes}</p>}</section></div>
}
