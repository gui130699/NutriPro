...
export const toGrams = (quantity: number, unit: Unit, food: Pick<Food, 'unit_weight_g' | 'portion_weight_g'>) => {
  if (unit === 'g' || unit === 'ml') return quantity
  if (unit === 'kg' || unit === 'l') return quantity * 1000
  if (unit === 'unidade') return quantity * Number(food.unit_weight_g ?? 0)
  return quantity * Number(food.portion_weight_g ?? 0)
}
export const calculateNutrients = (food: Food, quantity: number, unit: Unit): Nutrients & { grams: number } => {
  ...
  return { grams, calories: food.calories * factor, protein: food.protein * factor, carbs: food.carbs * factor, fat: food.fat * factor, fiber: food.fiber * factor, saturated_fat: (food.saturated_fat ?? 0) * factor, sugar: (food.sugar ?? 0) * factor, sodium: (food.sodium ?? 0) * factor }
}
